    ...
    if (typeof updateDoubanVisibility === 'function') {
        ...
        try {
            window.history.pushState(
                {},
                `TV-Show - 免费在线视频搜索与观看平台`,
                `/`
            );
            // 更新页面标题
            document.title = `TV-Show - 免费在线视频搜索与观看平台`;
        } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - TV-Show`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'TV-Show-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'TV-Show-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'TV-Show-Settings_' + times + '.json');
}
...
// 显示初始搜索结果（快速响应）
function showInitialResults(results, query) {
    ...
    if (searchResultsCount) {
        ...
        // 添加"快速响应"标识
        searchResultsCount.innerHTML += ' <span class="text-xs text-green-400">(快速响应)</span>';
    }
    ...
    // 显示快速响应提示
    showToast(`快速响应：已显示 ${results.length} 个结果，更多结果正在加载中...`, 'success');
}

// 更新搜索结果（异步加载的额外结果）
function updateSearchResults(allResults, query) {
    ...
    if (searchResultsCount) {
        ...
        // 移除"快速响应"标识，显示完整结果
        searchResultsCount.innerHTML = searchResultsCount.textContent.replace(' (快速响应)', '');
    }
    ...
    // 显示完整结果提示
    showToast(`搜索完成：共找到 ${allResults.length} 个结果`, 'success');
}
...
function showNoResults() {
    ...
    resultsDiv.innerHTML = `
        <div class="col-span-full text-center py-16">
            ...
            <p class="text-sm text-gray-500">请尝试其他关键词或更换数据源</p>
        </div>
    `;
    hideLoading();
}
